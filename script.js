const DATA={
  ...
  amys:{
    ...
    a:"#1677ff", a2:"#0b3d91", ey:"Amys Arms • UMZINTO",
...
function load(key){
  ...
  $("logo").classList.toggle("amys-logo",key==="amys");
  ...
  const wa=$("whatsapp");
  wa.href=d.whatsapp;
  wa.textContent="JOIN "+d.city+" WHATSAPP GROUP";

  const query=encodeURIComponent(d.address.replace(/\n/g,", "));
...
document.querySelectorAll(".tabs button").forEach(btn=>
  btn.addEventListener("click",()=>load(btn.dataset.key))
);
